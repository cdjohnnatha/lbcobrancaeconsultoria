...
const verifyPasswords = async (requestPassword, password) => {
  const encrypted = bcrypt.hashSync(requestPassword, bcrypt.genSaltSync(10));
  return await User.comparePasswords(encrypted, password);
};
...
const findById = async (id) => {
  try {
    const user = await User.findByPk(id, { attributes: User.Attributes, raw: true });
...
const update = async (params, { id }) => {
  try {
    ...
    const result = (await user.update(params, { attributes: User.Attributes }));
